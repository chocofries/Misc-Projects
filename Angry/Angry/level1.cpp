...
void Level1::Initialise(Game * _myGame, ShaderLoader * _shaderloader, AssetLoader * _assetLoader, InputManager * _inputManager, AudioManager* _audioManager)
{
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 10; j++)
		{
			...
			if (j < 5)
			{
				box = new Box(this, BOX_HOLLOW, 8.0f, 4.0f, 0.0f, false, false, &vecDebris);
			}
			else
			{
				box = new Box(this, BOX_SOLID, 8.0f, 4.0f, 0.0f, false, false);
			}
			box->SetX(20.0f + (i*16.0f));
	...
	for (int i = 0; i < 3; i++)
	{
		Ball* ball = new Ball(this, 4.0f, BALL_DASH, &vecMinis);
		ball->SetY(0.0f);
		ball->SetX(0.0f);
	...
	//Init Texts
	textLevel = new Texter("", "Assets/Fonts/BabyMetal.ttf", glm::vec3(-10.0f, 350.0f, 200.0f), shaderLoader, assetLoader, levelCamera, true);
	textLevel->SetColor(glm::vec3(1.0f, 0.0f, 0.0f));
	textLevel->SetScale(0.25f);
...
void Level1::Update()
{
	...
	{
		myGame->GetGameInstance().ResetLevels();
		myGame->GetGameInstance().LoadLevel(2);
	}
...
void Level1::ProcessKeyInput()
{
}

void Level1::CheckWinLose()
{
